...
static void LCD_writeNibbleOff(uint8_t nibble, uint8_t mode);
...
void LCD_off(void)
{
    uint8_t data = LCD_CLEAR;
    uint8_t mode = LCD_COMMAND;
    LCD_writeNibbleOff(data & 0xF0, mode);
    LCD_writeNibbleOff((data << 4) & 0xF0, mode);
    vTaskDelay(2 / portTICK_RATE_MS);                                   // This command takes a while to complete
}
